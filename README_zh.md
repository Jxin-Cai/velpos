...
- **项目规则** — 在 UI 中编辑 `CLAUDE.md` 版本和项目规则，支持版本间差异对比
...
- **Evolution** — 从会话中提取可复用经验，生成 `CLAUDE.md` 版本或规则草稿
...
| Memory 与演化 | `CLAUDE.md` 修订简化为 编辑→保存→应用，新增版本差异对比，Evolution 可生成规则草稿 | 项目指令迭代更快，变更清晰可见 |
...
所有弹窗（设置、定时任务、规则、Evolution、插件、Agent、Git、IM）均支持 `ESC` 关闭。快捷键系统采用优先级派发机制——弹窗始终优先于全局处理器。
...
脚本会自动检查前置依赖、创建环境配置文件、提示输入必要密钥并启动所有服务。生产部署会首次构建仅包含运行时和通用工具的基础镜像；Python 和 Node 项目依赖始终在应用构建中根据当前 lockfile 安装，lockfile 未变化时复用 Docker 缓存。更新仍执行 `./build/prod/deploy.sh`，如需主动刷新基础镜像可设置 `VELPOS_REBUILD_BASE_IMAGES=true`。完整的机器可读部署指南请参考 [doc/deploy-guide.md](./doc/deploy-guide.md)（适合让 Claude Code 自动完成部署）。
...
生产容器会继承部署宿主机的本地时区。定时任务按宿主机时区执行；如果宿主机时区不正确，请先修正宿主机时区再部署。
...
| **规则** | 直接在 UI 中编辑 `CLAUDE.md` 版本和项目规则，支持版本间差异对比 |
