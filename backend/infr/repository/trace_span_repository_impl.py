...
class TraceSpanRepositoryImpl(TraceSpanRepository):
    ...
    async def find_run_version(self, session_id: str, run_id: str) -> str:
        stmt = select(
            func.count(TraceSpanModel.id),
            func.max(TraceSpanModel.sequence),
            func.sum(TraceSpanModel.revision),
        ).where(
            TraceSpanModel.session_id == session_id,
            TraceSpanModel.run_id == run_id,
        )
        result = await self._session.execute(stmt)
        span_count, max_sequence, revision_total = result.one()
        return f"{int(span_count or 0)}:{int(max_sequence or 0)}:{int(revision_total or 0)}"
